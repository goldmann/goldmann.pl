---
title: "Logging with the WildFly Docker image"
date: 2014-07-18T21:45:00.10+01:00
draft: false
...
![Logging with Docker images](/images/docker-logging/wildfly.png)

Let’s go through each of these options, learn a litle about the it,
and modify the `jboss/wildfly` image use it.

> [!NOTE]
> I won’t talk about WildFly logging in general, in this blog post I’ll focus
...
> [refer to
> the documentation](https://docs.jboss.org/author/display/WFLY8/Logging+Configuration).

## TL;DR;

Don’t use `docker logs` in production. Mounting volumes from a host is simple
but could be tricky, especially if you run
[SELinux](http://www.projectatomic.io/docs/docker-and-selinux/). If you set
...
Examples are [available on GitHub](https://github.com/goldmann/wildfly-docker-logging).

## `docker logs`

The [`logs`
command](https://docs.docker.com/reference/commandline/cli/#logs) is a handy feature of Docker. If the process you run inside of the
...
[Mounting
a volume from the host](https://docs.docker.com/userguide/dockervolumes/#mount-a-host-directory-as-a-data-volume) and exposing it in the container is another way to store
...
> [!NOTE]
> The biggest issue with this approach is that **it is not
...
[a well
known user uid/gid](https://github.com/jboss/dockerfiles/blob/master/wildfly/Dockerfile) (`431/433`) to run the server. We can use this information
...
[SELinux and Docker on
the Project Atomic website](http://www.projectatomic.io/docs/docker-and-selinux/).
...
> [!NOTE]
> The same `wildfly-logs` user can be used for any `jboss/wildfly`
> containers, because each container has a `wildfly` user created with the
> particular uid/gid.

Read more about using Docker volumes in the
[documentation](https://docs.docker.com/userguide/dockervolumes/).
...
> [!NOTE]
> All files are [available on GitHub](https://github.com/goldmann/wildfly-docker-logging/tree/master/data-container/data).
...
Save the above snippet as `Dockerfile` and build it with `docker build --rm --tag=data .`.

> [!NOTE]
> The `CMD` instruction above is not a mistake. To use the exposed
...
a `wildfly` user created. As [I mentioned
above](#_let_s_make_it_writeable), the same user will be used to launch the Java processes and if we mount a
...
> [!NOTE]
> This is not strictly true if you try to export a path that is actually a
...
> [!NOTE]
> All files are [available on GitHub](https://github.com/goldmann/wildfly-docker-logging/tree/master/data-container/wildfly).
...
> [!NOTE]
> Do not forget to `chcon -t svirt_sandbox_file_t .` the directory where
...
[rsyslog](http://www.rsyslog.com/), or [logstash](http://logstash.net/). No
...
> [!NOTE]
> All files are [available on GitHub](https://github.com/goldmann/wildfly-docker-logging/tree/master/syslog/syslog).
...
> [!NOTE]
> I need to mention that the true power in this (or any other) logger
...
> [!NOTE]
> All files are [available on GitHub](https://github.com/goldmann/wildfly-docker-logging/tree/master/syslog/wildfly).
...
The [Docker links
feature](https://docs.docker.com/userguide/dockerlinks/) if a great way to make containers talk to each other without
...
[Getting access to the logs](#_getting_access_to_the_logs) section above.
...
comment or [file a ticket](https://github.com/jboss/dockerfiles/issues/new).
