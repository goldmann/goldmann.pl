...
In my
[last blog
post](/blog/2014/01/21/connecting-docker-containers-on-multiple-hosts/) I explained how to run a Docker installation across multiple hosts.
...
[IP
assignment issue I talked about before](/blog/2014/01/21/connecting-docker-containers-on-multiple-hosts/#_the_issue_ip_assignment).

## Set up

The set up is pretty similar to my
[earlier
work](/blog/2014/01/21/connecting-docker-containers-on-multiple-hosts/#_set_up).
...
Additionally I rewrote [my earlier script](https://gist.github.com/goldmann/8455702)
that prepares the virtual machines for me. I decided to
drop the use of [cloud-init](http://cloudinit.readthedocs.org/en/latest/)
entirely in favor of [libguestfs](http://libguestfs.org/) — a great Swiss
Army knife written by [Rich](http://rwmj.wordpress.com/). If you’re not
...
> [!NOTE]
> The first run of [`guestfish`](http://libguestfs.org/guestfish.1.html)
> can take a bit, since it creates the
> [supermin
> appliance](http://libguestfs.org/febootstrap.8.html#supermin_appliances). Also, if you run it in a virtualized environment it will
> perform a bit worse compared to bare metal.

The new scripts are avilable in the
[docker-dhcp repository on
GitHub](https://github.com/goldmann/docker-dhcp).

The logic of preparing and registering the image as a libvirt domain was split
into two scripts:

- [`prepare-image.sh`](https://github.com/goldmann/docker-dhcp/blob/master/prepare-image.sh) — update the system, modify the image,

- [`register-domain.sh`](https://github.com/goldmann/docker-dhcp/blob/master/register-domain.sh) — copy the image to a custom location and register a
...
[cloud.fedoraproject.org](https://cloud.fedoraproject.org/) and run the script
...
> [!NOTE]
> Executing the above script can take some time. It took about 5 minutes for me
...
> [!NOTE]
> This is just an example — you can expand the default values to run more
...
[`goldmann/fedora-dhcp`](https://index.docker.io/u/goldmann/fedora-dhcp/)
...
> [!NOTE]
> I hope this will change in the near future. One option is to make systemd
...
1.  Make [systemd](http://freedesktop.org/wiki/Software/systemd/) available
