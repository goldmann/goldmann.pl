---
title: "Connecting Docker containers on multiple hosts"
date: 2014-01-21T16:20:00.10+01:00
draft: false
tags: [ docker, fedora ]
---

> [!WARNING]
> This guide is based on an old version of Docker. The instructions you
> find below may be already removed or changed in the Docker codebase.

In my [previous blog
post](/blog/2014/01/16/running-fedora-cloud-images-on-kvm/) I talked about running the
[Fedora Cloud images](http://fedoraproject.org/en/get-fedora#clouds) on
a local KVM with libvirt. This was not a standalone task, but rather the preparation
for this blog post: running [Docker](http://www.docker.io/)
containers on multiple hosts **attached to the same network**.

I was asked in the comments on my
[WildFly cluster based
on Docker blog post](/2013/10/07/wildfly-cluster-using-docker-on-fedora/) if it would be possible to run a cluster on multiple
hosts. I found a
[very
nice tutorial](http://fbevmware.blogspot.com/2013/12/coupling-docker-and-open-vswitch.html) written by Franck Besnard. I’ve decided to set up a similar
environment on my own to see how/if it works.

I made a few changes to Franck’s set up:

1.  I’m not using the [pipework](https://github.com/jpetazzo/pipework) script
...
![Networking](/images/docker-network/network.png "Networking")

I used two VM’s (`host1` and `host2`) each with Fedora 20 as the operating system.
You can use [the script](https://gist.github.com/goldmann/8455702)
[I described in
my previous post](/blog/2014/01/16/running-fedora-cloud-images-on-kvm/) to create them. Later you can use the `virsh start host1`
command to run them.
...
To make networking easy I used the [Open
vSwitch](http://openvswitch.org/) software. I’m very new to it, but its flexibility and ease of use is
just impressive. I haven’t done any performance testing, though. [Maybe some day](http://en.wikipedia.org/wiki/Future).
...
1.  Every container run with Docker is attached to `docker0` bridge. This is a
    [regular bridge](http://www.tldp.org/HOWTO/BRIDGE-STP-HOWTO/) you can
...
3.  Additionally we need to connect together the networks from both hosts in
    ...
    [GRE tunnel](http://en.wikipedia.org/wiki/Generic_Routing_Encapsulation)
...
to assign the first *unused* address. It sounds valid, right? But it depends how
do you define *not used*. When Docker starts a container — the assigned IP is
added to a list of used IPs maintained by the Docker daemon. *Not used* IP in Docker’s
...
2. Even if we advertise manually (using
   [gratuitous
   ARP message](http://en.wikipedia.org/wiki/Address_Resolution_Protocol#ARP_announcements)) — the ARP table is not reliable enough since entries will be
...
> [Read more in the
> Linux kernel documentation](https://www.kernel.org/doc/Documentation/networking/ip-sysctl.txt) (search for `arp_accept`).
...
I prepared a [very ugly patch](https://gist.github.com/goldmann/8520776) for
...
[download it
from here](http://koji.fedoraproject.org/koji/taskinfo?taskID=6429484), if you want to give it a try.
