...
In this blog post I would like to touch the resource management topic for Docker
containers. Often it is very unclear how it works and what we can and
what we cannot do. Hope after reading this blog post resource management will
be a bit easier to understand.

NOTE: I assume that you run Docker on a systemd enabled operating system. If
...
different systemd versions. In doubt use systemd man pages for the system you
work with.
...
group processes running in the container. This allows to manage the resources
...
manager -- every process (not only inside of the container) will be placed in
a cgroups tree. You can see it yourself if you run the `systemd-cgls` command:
...
In the examples I'll use the `stress` tool that helps me to generate some load
...
=== Note on resource reporting tools

The tools you are used to use to report usage like `top`, `/proc/meminfo` and
...
If you want to quickly find which container (and not ony) uses most resources
...
now. But in case you want to get some detailed information about the usage (for
example you need to create nice graphs) you want to parse the
`/sys/fs/cgroup/...` directories. I'll show where to find useful files for each
resource I will talk about (look for _CGroups fs_ paragraphs below).
...
and both will use 100% CPU, the CPU time will be divided between the two
...
If we set one container's CPU shares to `512` it'll receive half of the CPU
...
necessary (for example when many processes start to use CPU heavily at the same time).

Of course it's not easy (and I would say impossible) to say how many resources
will be assigned to your process. It really depends how other processes will
behave and how many shares are assigned to them. 

=== Example: managing CPU shares of a container
...
Since I have 4 cores on my machine available -- this is how we can try to use all of
them:

----
$ docker run -it --rm stress -c 4
...
----
$ docker run -it --rm -c 512 stress -c 4
stress: info: [1] dispatching hogs: 4 cpu, 0 io, 0 vm, 0 hdd
----

image:/images/docker-resources/stress-half.png[alt="Containers using CPU", width="100%"]

As you can see -- the CPU is divided between the two containers in such way
that the first container uses ~60% of the CPU and the other ~30%. This seems to
be an expected result.

NOTE: The missing ~10% of the CPU was taken by GNOME, Chrome and my music player, if you wonder.

=== Attaching containers to cores

Besides limiting shares of the CPU we can do one more thing: define on which
processors (cores) the container's processes will be executed. For this purpose
we use the `--cpuset` switch of the `docker run` command.

To allow execution only on first core:

----
docker run -it --rm --cpuset=0 stress -c 1
----

To allow execution only on first two cores:

----
docker run -it --rm --cpuset=0,1 stress -c 2
----

You can of course mix the `--cpuset` with `-c`.

NOTE: Share enforcement will only takes place where the processes are run on
...
if they have different CPU share value set (once again -- I assume that only
  these two containers are running on the host).

=== Changing shares value for a running container

It is possible to change the value of shares for a running container (or any
other process, of course). You can directly interact with cgroups filesystem,
but since we have systemd we can leverage it to manage this too (since it
manages the processes anyhow).

For this purpose we'll use the `systemctl` command with the `set-property`
argument. Every new container created using `docker run` command will have
automatically assigned a systemd scope under which all processes will be
executed. To change the CPU share for all processes in the container we just
need to change it for the scope, like this:

----
$ sudo systemctl set-property docker-4be96b853089bc6044b29cb873cac460b429cfcbdd0e877c0868eb2a901dbf80.scope CPUShares=512
----

NOTE: Add `--runtime` to change the setting temporarily.
...
You can find all the information about the CPU under for specific container
...
NOTE: More information about these files you can find in the
...
Resource Management Guide]. This information is spread across cpu, cpuacct and cpuset sections.

=== Recap

A few things to remember

. CPU shares are just a number -- it's not related to the CPU speed
...
First thing that is pretty important is that a *container can use all of the
memory on the host with default settings*.

If you want to limit memory for all of processes inside of the container just
use the `-m` docker run switch. You can define the value in bytes or by adding
a suffix (`k`, `m` or `g`).

=== Example: managing memory shares of a container

You can use the `-m` switch like this:

----
$ docker run -it --rm -m 128m fedora bash
----

To show that the limitation actually works I'll use one more time my `stress` image. Consider following run:

----
$ docker run -it --rm -m 128m stress -m 1 --vm-bytes 128M --vm-hang 0
...
----
$ docker run -it --rm -m 128m stress -m 1 --vm-bytes 200M --vm-hang 0
stress: info: [1] dispatching hogs: 0 cpu, 0 io, 1 vm, 0 hdd
----

It works too. Surprising? Yes I agree.

We can find explanation for this in the
link:https://github.com/docker/libcontainer/blob/v1.2.0/cgroups/fs/memory.go#L39[libcontainer
source code] (Docker's interface to cgroups). We can read there that by default
...
The current Docker interface *does not allow* to specify how much (or disable
it entirely) swap should be allowed, so we need live with it for now.

With above information we can run our example again. This time we will try to
allocate over twice the amount of memory we assign. This should use the whole
memory, use the whole swap and then die.

----
$ docker run -it --rm -m 128m stress -m 1 --vm-bytes 260M --vm-hang 0
stress: info: [1] dispatching hogs: 0 cpu, 0 io, 1 vm, 0 hdd
...
250M`) it'll work just fine filling almost any memory with garbage.

WARNING: If we don't limit the memory by using `-m` switch the swap size will be unlimited too.
footnoteref:[limit-actually-exists,It's not technically true; there is a limit, but it's set to a
value that is not reachable in the systems we currently run. For example on my
laptop with 16GB of ram the valuse is `18446744073709551615` which is ~18.5
exabyte...]
...
*always use `-m` parameter* footnoteref:[systemd-memory,Or just use the `MemoryLimit` property.]. 
...
To limit read and write speec we can use the `BlockIOReadBandwidth` and
...
It took 3.8 sec to save 100MB of data which gives about 26MB/s. Let's try to limit the disk speed a bit.
...
You can also use the `nsenter` command to get this value, like this:
...
Now we can change the value of the `BlockIOWriteBandwidth` property, like this:
...
NOTE: Same applies to limiting read bandwith with the difference that you use
`BlockIOReadBandwidth` property.

=== Limiting disk space

As I mentioned before this is though topic. By default *you get 10GB of space
for each container*. Sometimes this is too much, sometimes we do not fit our
all data there. Unfortunately there is not much we can do about it now.

The only thing we can do is to change the default value for new containers. If we think that some other value (for example 5GB) is a beter fit in your case, you can do it by specifying the `--storage-opt` for the Docker daemon, like this:
...
some other things], but please keep in mind that it requires restarting Docker
...
pretty easy. The only thing that bothers me (an others too) is the limitation
where we cannot set a quota for disk usage. There is an
link:https://github.com/docker/docker/issues/3804[issue filled] upstream, eatch
it, comment.
