...
NOTE: Of course you may want to have a load balancer in front of this setup.
The link:http://mod-cluster.jboss.org/[mod_cluster] project may be a good
choice. The load balancer set up is not part of this guide.
...
The `User` parameter specifies the user that will be used to exeute the command.
This is optional but it's a good idea to *drop the privileges as fast as
possible*.
...
This file is very similar to the first one *with one, but very important
difference*: the `After` and `Requires` parameters specify the `docker-mariadb`
service. This is because we want to boot WildFly after the MariaDB database.

If you want to run WildFly multiple nodes -- just copy the file and edit appropriate
values (node name).
